	...
	: BackgroundService
{
		...
		foreach (var processor in _processors)
		{
			foreach (var symbol in processor.Symbols.Distinct(StringComparer.OrdinalIgnoreCase))
			{
				foreach (var interval in processor.Intervals.Distinct(StringComparer.OrdinalIgnoreCase))
				{
					...
					// Redis can survive a service restart and therefore keep indicator_state keys
					// that were created by an older application version without an expiry.
					// Refresh the expiry during startup as well as after every new snapshot so a
					// stale current-state key can never live forever just because the next candle
					// has not closed yet.
	...
	private static TimeSpan ResolveStateTtl(string interval)
	{
		...
		return ttl < TimeSpan.FromMinutes(2)
