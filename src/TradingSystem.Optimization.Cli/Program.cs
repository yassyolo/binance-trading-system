...
else
{
    var rows = await tuning.RunAsync(candidates, (o, _) 
        => Task.FromResult(engine.Run(candles, signals, o)), x => x.Metrics, weights, Int("top", 20));
...
int Int(string key, int fallback)  
    => int.TryParse(Get(key, string.Empty), out var value) ? value : fallback;

decimal Decimal(string key, decimal fallback)  
    =>  decimal.TryParse(Get(key, string.Empty), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out var value) 
        ? value 
        : fallback;
...
{ 
    ...
    { 
        ...
        var key = values[i][2..]; 
        var value = i + 1 < values.Length && !values[i+1].StartsWith("--")
            ? values[++i] 
            : "true"; 
