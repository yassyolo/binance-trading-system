...
public sealed class RedisSignalCooldownStore(IConnectionMultiplexer redis,  RedisKeyFactory keys,  IClock clock) : ISignalCooldownStore
{
    ...
    public async Task<TimeSpan?> GetRemainingAsync(string botName,  string symbol,  PositionSide side,  DateTime nowUtc,  CancellationToken ct)
    {
        ...
        if (!long.TryParse(value,  NumberStyles.Integer,  CultureInfo.InvariantCulture,  out var ms)) { await _db.KeyDeleteAsync(key); return null; }
        var remaining = DateTimeOffset.FromUnixTimeMilliseconds(ms).UtcDateTime-nowUtc;
        if (remaining>TimeSpan.Zero) return remaining;
        await _db.KeyDeleteAsync(key); return null;
    }
    public Task SetAsync(string botName, string symbol, PositionSide side, DateTime expiresAtUtc, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested(); var ttl = expiresAtUtc-clock.UtcNow; if(ttl<=TimeSpan.Zero) return Task.CompletedTask;
