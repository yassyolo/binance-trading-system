...
public sealed class PaperActivePositionProvider(IPaperTradingStore paperPositionStore)
{
    public async Task<IReadOnlyCollection<ActivePositionView>> GetAsync(string botName, string symbol, CancellationToken ct)
    {
        var positions = await paperPositionStore.QueryAsync(botName, symbol, PaperPositionStatus.Open, 0, 500, ct);
