      ...
      IRiskAdmissionLifecycle
{
    ...
    public async Task<RiskDecision> EvaluateOpenAsync(RiskEvaluationContext context, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Central risk is portfolio-wide. RiskEvaluationContext.ActivePositions is intentionally
            // scoped by TradingEngine to the current bot/symbol and therefore must not be used for
            // global, per-bot, per-symbol or notional portfolio limits.
            //
            // Force a fresh snapshot for each admission decision. The snapshot provider combines
            // Redis active positions with PostgreSQL Paper positions through
            ...
            if (snapshot.OpenPositions + reservedCount >= _options.MaximumOpenPositions)
                return RiskDecision.Block(
                    "MAX_OPEN_POSITIONS",
                    $"Maximum total open positions reached. Current = {snapshot.OpenPositions}, Reserved = {reservedCount}, Limit = {_options.MaximumOpenPositions}.");

            var bot = snapshot.Bots.FirstOrDefault(x =>
                x.BotName.Equals(context.Signal.BotName, StringComparison.OrdinalIgnoreCase));
            ...
            if (botPositions + reservedForBot >= _options.MaximumOpenPositionsPerBot)
                return RiskDecision.Block(
                    ...
                    $"Maximum positions for '{context.Signal.BotName}' reached. Current = {botPositions}, Reserved = {reservedForBot}, Limit = {_options.MaximumOpenPositionsPerBot}.");
            ...
            if (symbolPositions + reservedForSymbol >= _options.MaximumOpenPositionsPerSymbol)
                return RiskDecision.Block(
                    ...
                    $"Maximum positions for '{context.Signal.Symbol}' reached. Current = {symbolPositions}, Reserved = {reservedForSymbol}, Limit = {_options.MaximumOpenPositionsPerSymbol}.");

            var projectedTotalNotional =
                snapshot.GrossNotional + reservedNotional + candidateNotional;
            ...
            var symbolReservations = activeReservations
                .Where(x => x.Symbol.Equals(context.Signal.Symbol, StringComparison.OrdinalIgnoreCase))
                .ToArray();
            ...
            logger.LogInformation(
                "Risk admission allowed. SignalId = {SignalId}, Bot = {Bot}, Symbol = {Symbol}, Side = {Side}, CandidateNotional = {Notional}, OpenPositions = {OpenPositions}, ProjectedGross = {ProjectedGross}",
                context.Signal.SignalId,
                context.Signal.BotName,
                context.Signal.Symbol,
                context.Signal.Side,
