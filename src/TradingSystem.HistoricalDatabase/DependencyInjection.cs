...
{
    public static IServiceCollection AddHistoricalDatabase(this IServiceCollection services, IConfiguration configuration)
    {
        services.AddOptions<HistoricalDatabaseOptions>().Bind(configuration.GetSection(HistoricalDatabaseOptions.SectionName)).ValidateOnStart();
