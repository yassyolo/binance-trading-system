...
CREATE TABLE IF NOT EXISTS trading_dashboard.service_heartbeats(component varchar(96) PRIMARY KEY, status varchar(24) NOT NULL DEFAULT 'Healthy', last_seen_at_utc timestamptz NOT NULL, stale_after_seconds integer NOT NULL DEFAULT 60, details text, metadata jsonb NOT NULL DEFAULT '{}'::jsonb);
...
SELECT c.bot_name, c.runtime_status status, c.environment, c.signal_source, 
 ...
 coalesce(ls.strategy_version, 'unknown') strategy_version, h.last_seen_at_utc last_heartbeat_utc
