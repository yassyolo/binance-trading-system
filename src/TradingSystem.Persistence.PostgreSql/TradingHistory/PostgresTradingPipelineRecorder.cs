    ...
    : ITradingPipelineRecorder
{
    ...
    public Task UpsertPositionAsync(PositionHistoryRecord x, CancellationToken ct) 
        => Exec(
            ...
            on conflict(position_id) do update set
                ...
                strategy_version = 
                    case
                    when excluded.strategy_version is not null 
                        and excluded.strategy_version <> 'unknown'
    ...
    private async Task Exec(string sql, object args, CancellationToken ct)
    {
        ...
        await connection.ExecuteAsync(new CommandDefinition(sql, args, commandTimeout: factory.CommandTimeoutSeconds, cancellationToken: ct));
    }

    private static string Serialize(object? value) 
        => JsonSerializer.Serialize(value ?? new { }, Json);
