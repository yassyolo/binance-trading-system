	...
	: IPerformanceAnalyticsStore
{
	...
	public async Task SaveSnapshotAsync(PerformanceSnapshot s, CancellationToken ct = default)
	{
		...
		await connection.ExecuteAsync(new CommandDefinition(sql, new
		{
			...
			s.PeriodToUtc,
			s.Metrics.Signals,
			s.Metrics.OpenedPositions,
			s.Metrics.BlockedSignals,
			s.Metrics.ClosedPositions,
			s.Metrics.WinningPositions,
			s.Metrics.LosingPositions,
			s.Metrics.InitialBalance,
			s.Metrics.FinalBalance,
			s.Metrics.NetProfit,
			s.Metrics.ReturnPercent,
			s.Metrics.WinRatePercent,
			s.Metrics.ProfitFactor,
			s.Metrics.MaximumDrawdownAmount,
			s.Metrics.MaximumDrawdownPercent,
			s.Metrics.TotalFees,
			s.Metrics.Expectancy,
			s.Score
		}, cancellationToken: ct));
	}
		...
		try
		{
			...
			await connection.ExecuteAsync(new CommandDefinition(
				...
				new
				{
					...
					m.WinRatePercent,
					m.ProfitFactor,
					m.MaximumDrawdownAmount,
					m.MaximumDrawdownPercent,
					m.TotalFees,
					m.Expectancy,
					snapshot.Score
				},
			...
			await connection.ExecuteAsync(new CommandDefinition(
				"DELETE FROM trading.performance_trades WHERE run_id = @RunId;",
				new { run.RunId },
				transaction,
				cancellationToken: ct));
