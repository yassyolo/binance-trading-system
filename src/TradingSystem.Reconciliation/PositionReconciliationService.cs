	...
	IBotRuntimeConfigurationProvider runtimeConfigurations)
{
	...
	public async Task<ReconciliationRunResult> RunAsync(CancellationToken ct)
	{
		...
		// Paper bots are persisted in PostgreSQL and must never be reconciled against
		// the Binance account. A dedicated Paper reconciliation flow can be added separately.
		if (liveBots.Count == 0)
		{
