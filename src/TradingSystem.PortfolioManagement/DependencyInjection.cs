...
{
    public static IServiceCollection AddPortfolioManagement(this IServiceCollection services, IConfiguration configuration)
    {
        services.AddOptions<PortfolioOptions>().Bind(configuration.GetSection(PortfolioOptions.SectionName)).ValidateOnStart();
        services.AddSingleton<IValidateOptions<PortfolioOptions>, PortfolioOptionsValidator>();

        services.TryAddSingleton<IPortfolioPerformanceSource, NullPortfolioPerformanceSource>();
        services.TryAddSingleton<IPaperPortfolioPositionSource, NullPaperPortfolioPositionSource>();
