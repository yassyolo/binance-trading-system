...
public sealed class HealingPublisher(IBinanceOrdersSnapshotProvider snapshots, IRedisMessagePublisher publisher, IOptions<UserStreamServiceOptions> options, TimeProvider time, ILogger<HealingPublisher> logger)
{
    ...
    public async Task PublishAfterReconnectAsync(TimeSpan downtime, CancellationToken ct)
    {
        ...
        try
        {
            ...
            foreach(var rawSymbol in _o.HealingSymbols.Distinct(StringComparer.OrdinalIgnoreCase))
            {
                ...
                var msg = new HealingSnapshotMessage
                {
                    HubTimestampUtc = now.UtcDateTime, 
                    SnapshotTimestampUtc = now.UtcDateTime, 
                    DowntimeSeconds = Math.Round(downtime.TotalSeconds, 1), 
                    Symbol = symbol, 
                    NormalOrdersCount = s.NormalOrders.Length, 
                    AlgoOrdersCount = s.AlgoOrders.Length, 
                    TotalOrdersCount = ids.Count,
                    ActiveClientIds = ids, 
                    NormalOrders = s.NormalOrders, 
                    AlgoOrders = s.AlgoOrders
                };
    ...
    static IReadOnlyCollection<string> Collect(IEnumerable<JsonElement> normal, IEnumerable<JsonElement> algo)
    {
        ...
        foreach(var x in normal)
            Add(x, "clientOrderId", set);
        
        foreach(var x in algo)
            Add(x, "clientAlgoId", set);
    ...
    {
        if(e.TryGetProperty(name, out var p) && p.ValueKind==JsonValueKind.String && !string.IsNullOrWhiteSpace(p.GetString()))
            set.Add(p.GetString()!);
    }
}
