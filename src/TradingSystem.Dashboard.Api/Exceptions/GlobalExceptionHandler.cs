    ...
    : IExceptionHandler
{
    ...
    {
        ApiValidationException validation => new(StatusCodes.Status400BadRequest, "Validation failed", "https://trading-system/errors/validation",
            true,
            validation.Errors),
        ApiConflictException => new(StatusCodes.Status409Conflict, "Conflict", "https://trading-system/errors/conflict",
            true),
        ApiNotFoundException => new(StatusCodes.Status404NotFound, "Resource not found", "https://trading-system/errors/not-found",
            true),
        UnauthorizedAccessException => new(StatusCodes.Status403Forbidden, "Forbidden", "https://trading-system/errors/forbidden",
            false,
            PublicDetail: "You are not allowed to perform this operation."),
        PostgresException { SqlState: PostgresErrorCodes.UniqueViolation } => new(StatusCodes.Status409Conflict, "Duplicate request", "https://trading-system/errors/duplicate",
            false,
            PublicDetail: "A conflicting resource already exists."),
        OperationCanceledException when context.RequestAborted.IsCancellationRequested => new(499, "Request cancelled", "https://trading-system/errors/cancelled",
            false,
            PublicDetail: "The client cancelled the request."),
        _ => new(StatusCodes.Status500InternalServerError, "Unexpected server error", "https://trading-system/errors/internal",
            false,
