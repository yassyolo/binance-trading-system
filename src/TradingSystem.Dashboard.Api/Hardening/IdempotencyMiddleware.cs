...
public sealed class IdempotencyMiddleware(RequestDelegate next)
{
    ...
    public async Task InvokeAsync(HttpContext context,  ITradingDbConnectionFactory connections,  ILogger<IdempotencyMiddleware> logger)
    {
        ...
        if (!IsValidKey(key))
            throw new ApiValidationException(new Dictionary<string,  string[]> { [HeaderName]  =  ["A 16-128 character idempotency key is required for this operation."] });

        context.Request.EnableBuffering();
        var requestBody  =  await ReadBodyAsync(context.Request,  context.RequestAborted);
        context.Request.Body.Position  =  0;
        var requestHash  =  Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes($"{context.Request.Method}|{context.Request.Path}|{requestBody}")));
        ...
        var inserted  =  await connection.ExecuteAsync(new CommandDefinition(@"
            ...
            new { key,  actor,  method  =  context.Request.Method,  path  =  context.Request.Path.Value,  requestHash },  cancellationToken: context.RequestAborted));
        ...
        if (inserted == 0)
        {
            var existing  =  await connection.QuerySingleAsync<ExistingRequest>(new CommandDefinition(@"
                ...
                new { key,  actor },  cancellationToken: context.RequestAborted));

            if (!string.Equals(existing.RequestHash,  requestHash,  StringComparison.Ordinal))
                throw new ApiConflictException("The idempotency key was already used with a different request.");
            ...
            if (existing.Status == "Completed"  &&  existing.ResponseStatusCode.HasValue)
            {
                ...
                if (!string.IsNullOrWhiteSpace(existing.ResponseContentType)) context.Response.ContentType  =  existing.ResponseContentType;
                context.Response.Headers["X-Idempotent-Replay"]  =  "true";
                if (existing.ResponseBody is { Length: > 0 }) await context.Response.Body.WriteAsync(existing.ResponseBody,  context.RequestAborted);
                return;
            }
        ...
        try
        {
            ...
            var responseBytes  =  responseBuffer.Length <= MaximumBodyBytes ? responseBuffer.ToArray() : [];
            await connection.ExecuteAsync(new CommandDefinition(@"
                update trading_dashboard.api_idempotency_keys
                set status = 'Completed',  response_status_code = @statusCode,  response_content_type = @contentType, 
                    response_body = @responseBody,  completed_at_utc = now()
                where idempotency_key = @key and actor = @actor;", 
                new { key,  actor,  statusCode  =  context.Response.StatusCode,  contentType  =  context.Response.ContentType,  responseBody  =  responseBytes },  cancellationToken: context.RequestAborted));
        ...
        catch
        {
            try
            {
                await connection.ExecuteAsync(new CommandDefinition(@"
                    ...
                    new { key,  actor },  cancellationToken: CancellationToken.None));
            }
            catch (Exception cleanupError)
            {
                logger.LogWarning(cleanupError,  "Could not release idempotency key {IdempotencyKey}.",  key);
            }
            throw;
        }
        finally
    ...
    private static bool IsValidKey(string? key)  =>  key is { Length: >= 16 and <= 128 }  &&  key.All(c  =>  char.IsLetterOrDigit(c)  ||  c is '-' or '_' or '.');
    ...
    private sealed record ExistingRequest(string RequestHash,  string Status,  int? ResponseStatusCode,  string? ResponseContentType,  byte[]? ResponseBody);
}
