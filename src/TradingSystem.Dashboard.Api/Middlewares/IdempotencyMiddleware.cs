...
public sealed class IdempotencyMiddleware(RequestDelegate next)
{
    ...
    public async Task InvokeAsync(HttpContext context, ITradingDbConnectionFactory connections, ILogger<IdempotencyMiddleware> logger)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                logger.LogWarning("Response for idempotency key {IdempotencyKey} was {Length} bytes and was not cached.", key, responseBuffer.Length);
            }
        ...
        catch
        {
            ...
            {
                logger.LogWarning(cleanupEx,"Could not release idempotency key {IdempotencyKey}.", key);
            }
            throw;
        }
        finally
    ...
    private static Task RemoveExpiredAsync(System.Data.Common.DbConnection connection, string key, string actor, CancellationToken ct)
    ...
    private static Task ReleaseAsync(System.Data.Common.DbConnection connection, string key, string actor, CancellationToken ct) 
        => connection.ExecuteAsync(new CommandDefinition(@"
            delete from trading_dashboard.api_idempotency_keys
            where idempotency_key = @key and actor = @actor and status = 'Processing';",
            new { key, actor }, cancellationToken: ct));
