...
public sealed class BinanceUserStreamClient(IOptions<BinanceUserStreamOptions> options):IBinanceUserStreamClient
{
    ...
    public async Task RunAsync(string key, Func<string, CancellationToken, Task> onMessage, Func<CancellationToken, Task>? connected, CancellationToken ct)
    {
        ...
        {
            KeepAliveIntervalSeconds = _o.KeepAliveIntervalSeconds, 
            ReceiveBufferSizeBytes = _o.ReceiveBufferSizeBytes
        };
        
        using var socket = ClientWebSocketFactory.Create(wsOptions);
        
        var url = $"{_o.WebSocketBaseUrl.TrimEnd('/')}/ws/{Uri.EscapeDataString(key)}";
        
        await socket.ConnectAsync(new Uri(url), ct);if(connected is not null)await connected(ct);
