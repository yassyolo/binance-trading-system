...
public sealed class BinanceHistoricalCandleRangeSource(HttpClient httpClient) : IHistoricalCandleRangeSource
{
        ...
        CancellationToken cancellationToken  =  default)
    {
        ...
        while (cursor < end)
        {
            ...
            var url  =  $"/fapi/v1/klines?symbol = {Uri.EscapeDataString(normalizedSymbol)}&interval = {Uri.EscapeDataString(normalizedInterval)}&startTime = {cursor}&endTime = {end}&limit = {PageLimit}";
            ...
            foreach (var row in rows)
            {
                result.Add(new MarketCandle(
                    ...
                    ParseDecimal(row[4]),  ParseDecimal(row[5]),  true));
            }

            var next  =  rows[^1][0].GetInt64() + 1;
            if (next <= cursor  ||  rows.Length < PageLimit)
                break;
            cursor  =  next;
        }
