...
public sealed class Bot8014SignalGenerator(
    ...
    : ITradingSignalGenerator
{
    ...
    public ValueTask<GeneratedTradingSignal?> GenerateAsync(MarketIndicatorSnapshot x, CancellationToken ct)
    {
        ...
        if (!x.TryGet(IndicatorKeys.BollingerUpper, out var upper)
            || !x.TryGet(IndicatorKeys.BollingerLower, out var lower))
            return ValueTask.FromResult<GeneratedTradingSignal?>(null);

        var side = x.Close > upper ? "LONG" 
            : x.Close < lower ? "SHORT" 
            : null;
