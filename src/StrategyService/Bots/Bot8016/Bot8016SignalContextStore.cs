...
public sealed class Bot8016SignalContextStore
{
    ...
    public void Set(string signalId, Bot8016EntrySignal signal)
        => _signals[signalId] = signal;
    
    public bool TryGet(string signalId, out Bot8016EntrySignal signal) 
        => _signals.TryGetValue(signalId, out signal!);
   
    public void Remove(string signalId) 
        => _signals.TryRemove(signalId, out _);
}
