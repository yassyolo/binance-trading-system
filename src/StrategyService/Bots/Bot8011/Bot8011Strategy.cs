    ...
    IHasSignalCooldown
{
    ...
    public Task<StrategyDecision> DecideAsync(StrategyContext ctx, CancellationToken ct)
    {
        ...
        var oppositePositionIds = ctx.ActivePositions.Where(x => x.Side != side).Select(x => x.ShortId).ToArray();
        if (oppositePositionIds.Length > 0)
            return Task.FromResult(StrategyDecision.OpenAfterClosing(side, oppositePositionIds, $"Reverse signal: close {oppositePositionIds.Length} oppositePositionIds position(s)."));

        var sameSidePositionsCount = ctx.ActivePositions.Count(x => x.Side == side);
