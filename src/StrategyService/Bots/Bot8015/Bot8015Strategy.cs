    ...
    : ITradingStrategy, IHasSignalCooldown
{
    ...
    public Task<StrategyDecision> DecideAsync(StrategyContext context, CancellationToken ct)
    {
        ...
        var oppositePositionIds = context.ActivePositions.Where(position => position.Side != side)
            .Select(position => position.ShortId)
            .ToArray();

        if (oppositePositionIds.Length > 0)
            return Task.FromResult(StrategyDecision.OpenAfterClosing(side, oppositePositionIds, $"Reverse signal: close {oppositePositionIds.Length} opposite position(s)."));

        var sameSidePositionsCount = context.ActivePositions.Count(position => position.Side == side);
