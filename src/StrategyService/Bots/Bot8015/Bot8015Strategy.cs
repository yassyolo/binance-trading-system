	...
	: ITradingStrategy, IHasSignalCooldown
{
	...
	public Task<StrategyDecision> DecideAsync(StrategyContext context,CancellationToken cancellationToken)
	{
		...
		logger.LogInformation("BOT8015 evaluating signal. Symbol = {Symbol}, Side = {Side}, Source = {Source}, ActivePositions = {ActivePositions}",
		...
		{
			logger.LogInformation("BOT8015 blocked LONG because LONG is disabled.");
			return Task.FromResult(StrategyDecision.Block(side, "LONG is disabled."));
		}

		if (side == PositionSide.Short && !_options.EnableShort)
		{
			logger.LogInformation("BOT8015 blocked SHORT because SHORT is disabled.");
			return Task.FromResult(StrategyDecision.Block(side, "SHORT is disabled."));
		}

		var oppositePositionIds = context.ActivePositions.Where(p => p.Side != side)
			.Select(p => p.ShortId)
			.ToArray();

		if (oppositePositionIds.Length > 0)
		{
			logger.LogInformation("BOT8015 reverse signal will close {Count} opposite positions. Side = {Side}", oppositePositionIds.Length, side);
		...
		var sameSidePositionsCount = context.ActivePositions.Count(p => p.Side == side);
		if (sameSidePositionsCount >= _options.OrderSideLimit)
		{
			logger.LogInformation("BOT8015 signal blocked by side limit. Side = {Side}, Count = {Count}, Limit = {Limit}", side, sameSidePositionsCount, _options.OrderSideLimit);
			return Task.FromResult(StrategyDecision.Block(side, $"ORDER_SIDE_LIMIT reached ({sameSidePositionsCount}/{_options.OrderSideLimit})."));
		}

		logger.LogInformation("BOT8015 accepted signal. Symbol = {Symbol}, Side = {Side}", context.Signal.Symbol, side);

		return Task.FromResult(StrategyDecision.Open(side, $"BOT8015 accepted {side} signal."));
	}
}