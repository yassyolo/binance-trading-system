    ...
    BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        {
            foreach(var p in (await store.GetAllAsync(o.BotName, ct)).Where(x => !x.Closed && x.TpExecuted && x.Stop3Created && !x.Stop3Pending && !x.TrailingInProgress && x.Stop3Current.HasValue && x.RemainingQuantity>0))
            {
                ...
                var candidate = p.Side == PositionSide.Long && price >= (p.Stop3Current!.Value+o.Stop3TrailingStep)
                    ? p.Stop3Current.Value+o.Stop3TrailingBuffer
                    :p.Side==PositionSide.Short && price <= (p.Stop3Current!.Value-o.Stop3TrailingStep)
                    ? p.Stop3Current.Value-o.Stop3TrailingBuffer
                    : p.Stop3Current!.Value;
                
                if(candidate==p.Stop3Current)
                    continue;                
               
                await Move(p.ShortId, candidate, ct);
            }
            await Task.Delay(TimeSpan.FromSeconds(o.TrailingCheckIntervalSeconds), ct);
        }
    }
    ...
    async Task Move(string id, decimal candidate, CancellationToken ct)
    {
        ...
        if(p is null || p.Closed || string.IsNullOrWhiteSpace(p.Stop3OrderId))
            return;p.TrailingInProgress = true;p.Stop3NewPending = candidate;
