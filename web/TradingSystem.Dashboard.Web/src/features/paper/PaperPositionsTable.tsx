...
import { decimal, sideTone, utcDate } from '@/features/paper/paper-formatters'
import { usePaperPositions } from '@/features/paper/paper.queries'
import type { PaperTradingPositionDto } from '@/types/paper-trading'

function sideLabel(side: PaperTradingPositionDto['side']) {
  if (side === 1) 
    return 'Long'
  if (side === 2) 
    return 'Short'
  ...
  [
    {
      ...
      render: (position) => (
        <div className="max-w-[180px]">
          <p className="truncate font-mono text-xs text-[var(--color-text-primary)]" title={position.positionId}>
    ...
    },
    {
      ...
      header: 'Entry',
      align: 'right',
      render: (position) => decimal(position.entryPrice, 4),
    },
    {
      key: 'tp',
      header: 'Take Profit',
      align: 'right',
      render: (position) => decimal(position.takeProfitPrice, 4),
    },
    {
      key: 'sl',
      header: 'Stop Loss',
      align: 'right',
      render: (position) => decimal(position.stopLossPrice, 4),
    },
    {
      key: 'fee',
      header: 'Entry Fee',
      align: 'right',
      render: (position) => decimal(position.entryFee, 6),
    },
    {
      key: 'source',
      header: 'Source',
    ...
    },
    {
      key: 'opened',
      header: 'Opened UTC',
      render: (position) => ( <span className="whitespace-nowrap text-xs"> {utcDate(position.openedAtUtc)} </span>
      ),
    },
  ]
